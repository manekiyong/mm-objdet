...
# ADDED
def get_embedding(x, model, img, im0):
    # Strip classifier head from model
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            b = xyxy2xywh(d[:, :4])  # boxes
            b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # rectangle to square
            # REMOVED, why do you pad the bounding box tho
            # b[:, 2:] = b[:, 2:] * 1.3 + 30  # pad
            d[:, :4] = xywh2xyxy(b).long()
            ...
            for j, a in enumerate(d):  # per item
                ...
                ims.append(im_pp)
            ims = torch.stack(ims)
            embs = model(torch.Tensor(ims).to(d.device))
            embs = embs.reshape((embs.size()[0],embs.size()[1]))
